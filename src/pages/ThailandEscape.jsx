...
const HERO = {
  ...
  lead:
    'Five easy days across Pattaya’s beaches and Bangkok’s temples — the Alcazar cabaret, a Coral Island speedboat run, Safari World and the city’s golden shrines, with private transfers throughout. Just your party, your pace. 🇹🇭',
...
const REVIEWS = [
  { name: 'The Kapoor Family', age: '2 kids', trip: 'Thailand · Pattaya & Bangkok', rating: 5, span: 'tall',
  ...
  { name: 'Neha & Arjun', age: '', trip: 'Thailand · Honeymoon', rating: 5, span: 'tall',
  ...
  { name: 'Sameer Gupta', age: '', trip: 'Thailand · Temples & city', rating: 5, span: 'tall',
  ...
  { name: 'The Shah Family', age: '4 travelling', trip: 'Thailand · Escape', rating: 5, span: 'tall',
  ...
  { name: 'Riya & friends', age: 'Group of 5', trip: 'Thailand · Spring 2026', rating: 5, span: 'tall',
  ...
  { name: 'Anita & Vikram', age: '', trip: 'Thailand · Anniversary', rating: 5, span: 'tall',
...
const ASSURE = [
  { icon: BadgeCheck, label: '4.8 / 5 rating', note: 'From 2,400+ verified travellers' },
...
export default function ThailandEscape() {
  ...
  return (
    <>
      <div className="lx2i lxjt lxjt2 lxjt3 lxjt4 lxjt5 tha">
        ...
        <main id="main">
          ...
          {/* ============ DATES & PRICES / BOOK ============ */}
          <section className="lxjt-book tha-book-section" id="dates">
            <div className="lx2i-container">
              <div className="lxjt-head lxjt-head--center lx2i-reveal">
                <span className="lx2i-eyebrow">// FIXED GROUP DEPARTURES · 2026</span>
                <h2 className="lx2i-h2">Book now &mdash; <strong>pick your date</strong></h2>
              </div>

              {/* Urgency / high-demand strip */}
              <div className="tha-urgency lx2i-reveal">
                <span className="tha-urgency__ic"><Flame size={20} /></span>
                <span className="tha-urgency__tx">
                  <strong>These 2026 departures are booking fast</strong>
                  <span>23 travellers viewed this tour in the last 24 hours &middot; 3 groups booked this week</span>
                </span>
                <span className="tha-urgency__live"><span className="tha-urgency__pulse" aria-hidden="true" /> {seatText(DEPARTURES.find((d) => d.level === 'error') || DEPARTURES[0])} on the earliest date</span>
              </div>

              <div className="tha-book2">
                {/* ---- LEFT: the selection (dates list + checkout) ---- */}
                <div className="tha-book2__main lx2i-reveal">
                  <div className="tha-book2__mainhead">
                    <div>
                      <span className="tha-lbl">Select a date</span>
                      <h3 className="tha-book2__h3">Available departures</h3>
                    </div>
                    <span className="tha-book2__count">{DEPARTURES.length} dates · 5 days / 4 nights</span>
                  </div>

                  <ul className="tha-deps" role="radiogroup" aria-label="Choose a departure date">
                    {DEPARTURES.map((d, i) => {
                      const on = selectedDep === i;
                      return (
                        <li key={d.date} className={`tha-dep ${on ? 'is-on' : ''}`}>
                          <button
                            type="button"
                            role="radio"
                            aria-checked={on}
                            className="tha-dep__btn"
                            onClick={() => setSelectedDep(i)}
                          >
                            <span className="tha-dep__radio" aria-hidden="true"><Check size={13} strokeWidth={3} /></span>
                            <span className="tha-dep__main">
                              <span className="tha-dep__top">
                                <span className="tha-dep__date">{d.date}</span>
                                {d.badge && <span className={`tha-dep__badge ${d.badge === 'Best price' ? 'tha-dep__badge--best' : ''}`}>{d.badge}</span>}
                              </span>
                              <span className="tha-dep__meta">{d.dow} &middot; {d.ret}</span>
                            </span>
                            <span className="tha-dep__right">
                              <span className="tha-dep__price">{inr(d.price)} <small>pp</small></span>
                              <span className={`tha-dep__tag tha-dep__tag--${d.level}`}>
                                <span className="tha-dep__dot" aria-hidden="true" />{seatText(d)}
                              </span>
                            </span>
                          </button>
                          {on && (
                            <div className="tha-dep__detail">
                              <span className="tha-dep__detailrow">
                                <Users size={14} /> {travellers} {travellers === 1 ? 'traveller' : 'travellers'} × {inr(d.price)}
                                <strong>= {inr(d.price * travellers)}</strong>
                              </span>
                              <span className="tha-dep__detailnote">
                                {d.level === 'ok'
                                  ? `Good availability — ${d.seats} of 20 seats open on this departure.`
                                  : `Hurry — only ${d.seats} of 20 seats remain on this departure.`}
                              </span>
                            </div>
                          )}
                        </li>
                  ...
                  {/* checkout controls */}
                  <div className="tha-checkout">
                    <div className="tha-travellers">
                      <div className="tha-checkout__row">
                        <span className="tha-trav-lbl"><span className="tha-lbl">Adults</span><small>Age 12+</small></span>
                      ...
                      <div className="tha-checkout__row">
                        <span className="tha-trav-lbl"><span className="tha-lbl">Children</span><small>Age 2&ndash;11</small></span>
                        <div className="lxjt-stepper">
                    ...
                    <div className="lxjt-due">
                      ...
                      <span className="lxjt-due__sub">{payMode === 'deposit' ? `Balance ${inr(balance)} due 45 days before travel` : `${travellers} × ${inr(pp)}, paid in full`}</span>
                    </div>

                    <button type="button" className="lx2i-btn lx2i-btn--primary lx2i-btn--lg lxjt-card__go">
                      Book {dep.date} &mdash; {travellers === 1 ? 'reserve my seat' : `reserve ${travellers} seats`} <ArrowRight size={17} />
                    </button>
                    <p className="lxjt-card__secure"><Lock size={13} /> Secure encrypted checkout · deposit protected · instant confirmation</p>
                  </div>
                </div>

                {/* ---- RIGHT: reassurance (sticky) ---- */}
                <aside className="tha-book2__aside lx2i-reveal" style={{ '--d': '.08s' }}>
                  <div className="tha-aside__head">
                    <span className="lx2i-eyebrow">// WHY BOOK WITH US</span>
                    <h3 className="tha-book2__h3">Book with confidence</h3>
                  </div>
                  <ul className="lxjt-book__perks">
                    <li><Check size={16} strokeWidth={2.4} /> Escorted small group — never more than 20 travellers</li>
                    <li><Check size={16} strokeWidth={2.4} /> Hotels, breakfast, key meals, sightseeing and all transfers included</li>
                    <li><Check size={16} strokeWidth={2.4} /> A tour manager &amp; local representative with you throughout</li>
                  </ul>
                  ...
                  <div className="tha-trust">
                    <span className="tha-trust__item"><BadgeCheck size={16} /> <span><strong>4.8/5</strong> · 2,400+ reviews</span></span>
                    <span className="tha-trust__item"><ShieldCheck size={16} /> Financially protected</span>
                    <span className="tha-trust__item"><Repeat size={16} /> Free date changes</span>
                    <span className="tha-trust__item"><Headset size={16} /> 24×7 human support</span>
                  </div>
                  <div className="tha-pay-methods">
                    <span><Lock size={12} /> Visa</span>
          ...
          <section className="tha-assure">
            <div className="lx2i-container">
              <div className="tha-assure__grid">
                {ASSURE.map(({ icon: Icon, label, note }) => (
          ...
          <section className="lxjt-review" id="reviews">
            <div className="lx2i-container">
              <div className="lxjt-head lxjt-head--center lxjt-head--light lx2i-reveal">
                ...
                <p className="lxjt-lead lxjt-lead--light">The moments that made each trip &mdash; straight from the families, couples and groups who lived them.</p>
              </div>
