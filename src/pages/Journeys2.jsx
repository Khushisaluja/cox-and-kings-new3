...
/* Plus / minus stepper for the number of travellers. */
function Stepper({ value, min, max, onChange }) {
  return (
    <div className="j2-stepper">
      ...
      <span className="j2-step-val" aria-live="polite">
        {value} <small>{value === 1 ? 'traveller' : 'travellers'}</small>
      </span>
      <button
        type="button" className="j2-step-btn"
...
export default function Journeys2() {
  ...
  const [people, setPeople] = useState(() => {
    const n = parseInt(params.get('people'), 10);
  ...
  useEffect(() => {
    ...
    if (people !== 2) next.people = String(people);
    if (travelDate) {
  ...
  }, [query, regions, styles, budgetMax, people, travelDate, flex, paces]);
  ...
  const clearAll = () => {
    ...
    setBudgetMax(PRICE_MAX); setTravelDate(null); setFlex(0); setPeople(2);
  };
  ...
  const filterBody = (
    <>
      ...
      <FilterField id="people" title="Travellers" icon={Users}>
        <Stepper value={people} min={1} max={12} onChange={setPeople} />
      </FilterField>
