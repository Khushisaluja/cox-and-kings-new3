...
import {
  ...
  Hotel, Utensils, TrainFront, Compass, ChevronLeft,
} from 'lucide-react';
...
/* ============================================================
   ...
   Route: /tour-detail-japan. Opens from the Japan package card on
   the Luxe2-Improved homepage. Visual language inherited wholesale
   from Luxe2Improved (lx2i-* tokens, glass, Zodiak serif, Voyager-
   Blue + warm paper). Self-contained + fully responsive.
   ============================================================ */
...
/* Header nav — same shape as the homepage (hover dropdowns). Cross-page
   links resolve to homepage sections; in-page (#) links smooth-scroll here. */
const NAV = [
...
const HERO = {
  ...
  image: '1712554009719-68e5aa9d6ba3',
  facts: [
...
/* Photo gallery — moments from the journey (opens in a lightbox). */
const GALLERY = [
  { id: '1522383225653-ed111181a951', cap: 'Cherry blossoms over a temple gate' },
  { id: '1480796927426-f609979314bd', cap: 'Shibuya crossing, Tokyo' },
  { id: '1493976040374-85c8e12f0c0e', cap: 'A quiet pagoda in Kyoto' },
  { id: '1533050487297-09b450131914', cap: 'Wandering the temple gardens' },
  { id: '1490806843957-31f4c9a91c65', cap: 'Mt. Fuji from the lake, Hakone' },
  { id: '1524413840807-0c3cb6fa808d', cap: 'Lanterns in the Gion lanes' },
  { id: '1590559899731-a382839e5549', cap: 'Dotonbori after dark, Osaka' },
  { id: '1528360983277-13d401cdc186', cap: 'Torii path at first light' },
];
...
const ITINERARY = [
  ...
  { day: 'Day 03', title: 'Tokyo at leisure', body: 'A free day to wander — teamLab’s digital art, the Tsukiji outer market, or a quiet garden. Your guide has ideas for every pace.' },
  { day: 'Day 04', title: 'Hakone & Mt. Fuji', body: 'Bullet train to Hakone. A lake cruise and ropeway with Mt. Fuji views, then a night in an onsen ryokan with a kaiseki dinner.' },
  { day: 'Day 05', title: 'Into Kyoto', body: 'Travel on to Kyoto by rail. A first evening walk through the lantern-lit lanes of Gion, Japan’s geisha district.' },
  { day: 'Day 06', title: 'Kyoto’s temples', body: 'Early-access to Fushimi Inari before the crowds, the golden Kinkaku-ji, and a private tea ceremony in a machiya townhouse.' },
  { day: 'Day 07', title: 'Nara day trip', body: 'A day in Nara — the great bronze Buddha of Todai-ji and the free-roaming, bowing deer of Nara Park.' },
  { day: 'Day 08', title: 'Arashiyama & bamboo', body: 'The towering Arashiyama bamboo grove at first light, the Iwatayama monkeys, and a riverside lunch.' },
  { day: 'Day 09', title: 'Kyoto at leisure', body: 'A free day. Shop Nishiki market, revisit a favourite temple, or simply rest before the journey south.' },
  { day: 'Day 10', title: 'Kyoto → Osaka', body: 'A short hop to Osaka. An evening food walk through the neon of Dotonbori — takoyaki, okonomiyaki and street-side ramen.' },
  { day: 'Day 11', title: 'Osaka', body: 'Osaka Castle and the city at your own pace, with a final group dinner to toast the journey.' },
  { day: 'Day 12', title: 'Depart for home', body: 'Transferred to Kansai airport for your flight home, with a little more Japan in you than when you arrived.' },
];
...
/* Photo-first traveller reviews (families & groups). Portrait IDs reuse the
   homepage's verified traveller photos so they always load and stay on-brand. */
const REVIEWS = [
...
/* Booking terms — a few key points shown up front; the full set opens in a modal.
   Content adapted from coxandkings.com/group-tours/essence-japan-with-hakone. */
const TERMS_SUMMARY = [
...
export default function JapanTourLuxe() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [chatOpen, setChatOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [openDay, setOpenDay] = useState(-1);       // itinerary starts fully collapsed
  ...
  const [lightbox, setLightbox] = useState(null);   // gallery index or null
  ...
  useEffect(() => {
    const lock = menuOpen || callbackOpen || termsOpen || lightbox !== null;
    document.body.style.overflow = lock ? 'hidden' : '';
    return () => { document.body.style.overflow = ''; };
  }, [menuOpen, callbackOpen, termsOpen, lightbox]);
  ...
  return (
    <>
      <div className="lx2i lxjt">
        ...
        <main id="main">
          ...
          <section className="lxjt-hero">
            ...
            <div className="lx2i-container lxjt-hero__inner">
              ...
              <div className="lxjt-hero__content lx2i-fade" style={{ '--d': '.1s' }}>
                ...
                {/* One clear primary action — dates & prices. Deeper navigation lives in the sub-nav below. */}
          ...
          <section className="lxjt-section" id="highlights">
            <div className="lx2i-container">
              ...
              {/* Key inclusions — what's in the price, at a glance */}
          ...
          {/* ============ DAY BY DAY ============ */}
          <section className="lxjt-section" id="itinerary">
            <div className="lx2i-container lxjt-narrow">
              <div className="lxjt-head lxjt-head--center lx2i-reveal">
                <span className="lx2i-eyebrow">// THE FULL PICTURE</span>
                <h2 className="lx2i-h2">Day by <strong>day</strong></h2>
                <p className="lxjt-lead">Tap any day to open it. Everything below is included in your price.</p>
              </div>
              <div className="lxjt-itin lx2i-reveal">
                {ITINERARY.map((d, i) => {
                  const open = openDay === i;
                  return (
                    <div key={d.day} className={`lxjt-day ${open ? 'is-open' : ''}`}>
                      <button type="button" className="lxjt-day__head" aria-expanded={open} onClick={() => setOpenDay(open ? -1 : i)}>
                        <span className="lxjt-day__label">{d.day}</span>
                        <span className="lxjt-day__title">{d.title}</span>
                        <span className="lxjt-day__icon" aria-hidden="true">{open ? <Minus size={18} /> : <Plus size={18} />}</span>
                      </button>
                      <div className="lxjt-day__panel"><div className="lxjt-day__panelin"><p>{d.body}</p></div></div>
                    </div>
                  );
                })}
              </div>
          ...
          {/* ============ PHOTO GALLERY (the journey in pictures) ============ */}
          <section className="lxjt-section lxjt-gallery" id="gallery">
            <div className="lx2i-container">
              <div className="lxjt-head lxjt-head--center lx2i-reveal">
                <span className="lx2i-eyebrow">// A LOOK AHEAD</span>
                <h2 className="lx2i-h2">The journey in <strong>pictures</strong></h2>
                <p className="lxjt-lead">A taste of the twelve days to come. Tap any photo to view it full-size.</p>
              </div>
              <div className="lxjt-gal__grid lx2i-reveal">
                {GALLERY.map((g, i) => (
                  <button
                    key={g.id}
                    type="button"
                    className={`lxjt-gal__item lxjt-gal__item--${i}`}
                    onClick={() => setLightbox(i)}
                    aria-label={`View photo: ${g.cap}`}
                    style={{ backgroundImage: `url(${sizedUnsplash(g.id, 700)})` }}
                  >
                    <span className="lxjt-gal__cap">{g.cap}</span>
                    <span className="lxjt-gal__zoom" aria-hidden="true"><Plus size={16} /></span>
                  </button>
                ))}
              </div>
            </div>
        ...
        {/* ============ GALLERY LIGHTBOX ============ */}
        {lightbox !== null && (
          <div className="lxjt-lb" role="dialog" aria-modal="true" aria-label={`Photo: ${GALLERY[lightbox].cap}`}>
            <div className="lxjt-lb__scrim" onClick={() => setLightbox(null)} />
            <button className="lxjt-lb__close" aria-label="Close" onClick={() => setLightbox(null)}><X size={22} /></button>
            <button className="lxjt-lb__arrow lxjt-lb__arrow--prev" aria-label="Previous photo" onClick={() => setLightbox((i) => (i - 1 + GALLERY.length) % GALLERY.length)}><ChevronLeft size={26} /></button>
            <figure className="lxjt-lb__stage">
              <img src={sizedUnsplash(GALLERY[lightbox].id, 1600)} alt={GALLERY[lightbox].cap} />
              <figcaption>{GALLERY[lightbox].cap}<span>{lightbox + 1} / {GALLERY.length}</span></figcaption>
            </figure>
            <button className="lxjt-lb__arrow lxjt-lb__arrow--next" aria-label="Next photo" onClick={() => setLightbox((i) => (i + 1) % GALLERY.length)}><ChevronRight size={26} /></button>
          </div>
        )}
