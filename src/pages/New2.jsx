/* ============================================================
   New2 — a second version of the /new composite page (route /new2).

   Same assembled sections as /new, with two focused UI/UX upgrades:
   · the intro "assurance points" (senior pacing, dietary meals,
     multi-gen travel, visa support) are rebuilt as an editorial
     card grid instead of a flat checklist  → .nx2-assure
   · the heritage block gains an "Accolades & Achievements" band
     showing the brand's real awards  → .nx2-awards

   The original /new page is left completely untouched.
   ============================================================ */
/* ============================================================
...
import {
  ...
  UtensilsCrossed, Stamp, Trophy, Medal, BadgeCheck,
} from 'lucide-react';
import {
...
/* ---- Intro "assurance" grid — the four reassurance points, rebuilt as
   editorial cards with an icon, a headline and a supporting line. Keeps
   the exact labels from ASSURANCE, adds context that earns the space. ---- */
const NX2_ASSURE = [
  { icon: Gauge, title: 'Senior-friendly pacing', desc: 'Gentle mornings, unhurried days and comfortable distances — kind on every generation.' },
  { icon: UtensilsCrossed, title: 'Indian & dietary meals', desc: 'Jain, vegetarian and special diets arranged, on request, wherever you travel.' },
  { icon: Users, title: 'Multi-generational travel', desc: 'From toddlers to grandparents — itineraries that keep the whole family happy.' },
  { icon: Stamp, title: 'Visa support', desc: 'Paperwork prepared and checked by our team — a 90% approval track record.', stat: '90%', statLabel: 'visa success' },
];

/* ---- Accolades & Achievements — real Cox & Kings awards, shown as a
   trophy band inside the heritage section (content from the brief). ---- */
const NX2_AWARDS = [
  { icon: Trophy, title: "India's Favourite Tour Operator", org: 'Condé Nast Traveller Readers', years: '2016 · 2017 · 2022' },
  { icon: Medal, title: 'PATA Gold Award', org: 'Pacific Asia Travel Association', years: '2015' },
  { icon: Award, title: "Asia's Leading Luxury Tour Operator", org: 'World Travel Awards', years: '2015 – 2019' },
  { icon: BadgeCheck, title: 'Best Luxury Travel Curator', org: "Travel + Leisure India's Best Awards", years: '2017' },
];
      ...
      <div className="h26-sheet">
        ...
        <section className="h26-intro" ref={scatterRef} onMouseMove={onScatterMove} onMouseLeave={onScatterLeave}>
          ...
          <div className="h26-intro-copy">
            ...
            <ul className="nx2-assure" aria-label="What we take care of for you">
              {NX2_ASSURE.map((a, i) => {
                const Icon = a.icon;
                return (
                  <Reveal key={a.title} as="li" className="nx2-assure-card" delay={0.34 + i * 0.08} y={18}>
                    <span className="nx2-assure-ic"><Icon size={20} strokeWidth={1.6} aria-hidden="true" /></span>
                    <div className="nx2-assure-body">
                      <h3 className="nx2-assure-title">{a.title}</h3>
                      <p className="nx2-assure-desc">{a.desc}</p>
                    </div>
                    {a.stat && (
                      <span className="nx2-assure-stat" aria-hidden="true">
                        <strong>{a.stat}</strong><small>{a.statLabel}</small>
                      </span>
                    )}
                  </Reveal>
                );
        ...
        <div className="lx2i">
          ...
          {/* ACCOLADES — award band inside the heritage block (New2) */}
          <section className="nx2-awards" aria-labelledby="nx2-awards-title">
            <div className="lx2i-container">
              <div className="nx2-awards-head lx2i-reveal">
                <span className="lx2i-eyebrow">ACCOLADES &amp; ACHIEVEMENTS</span>
                <h2 className="lx2i-h2" id="nx2-awards-title">Celebrated worldwide,<br /><strong>trusted for generations</strong></h2>
                <p className="nx2-awards-sub">Recognised by the travel industry and, more importantly, by the travellers who keep coming back.</p>
              </div>
              <ul className="nx2-awards-grid">
                {NX2_AWARDS.map((a, i) => {
                  const Icon = a.icon;
                  return (
                    <li
                      key={a.title}
                      className="nx2-award lx2i-reveal"
                      style={{ '--d': `${(i % 4) * 0.09}s` }}
                    >
                      <span className="nx2-award-medal"><Icon size={26} strokeWidth={1.4} aria-hidden="true" /></span>
                      <span className="nx2-award-years">{a.years}</span>
                      <h3 className="nx2-award-title">{a.title}</h3>
                      <p className="nx2-award-org">{a.org}</p>
                    </li>
                  );
                })}
              </ul>
