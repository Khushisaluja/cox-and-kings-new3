/* ============================================================
   Journeys4Group — the GROUP-TOURS entry into the Cox & Kings listing.

   A sibling of /journeys4. Below the hero it is the SAME page — the same
   full catalogue and the same open LEFT filter rail (Destination, Budget,
   Travellers, Dates, Pace, Trip style), the same filterable/sortable grid,
   badges and pill language, sharing Journeys4.css. Only two things differ:
   the hero speaks to group travellers ("Group Tours"), and the page opens
   with the "Group Tour" trip-style filter already selected — so it lands on
   group departures but a visitor can widen or change the filters freely.
...
export default function Journeys4Group() {
  ...
  /* Below the hero this page is identical to /journeys4 — same full catalogue,
     same sidebar. The one difference is that it opens with the "Group Tour"
  ...
  /* "Group Tours" is just /journeys4 opened on the Group Tour style. The moment
     a visitor removes that defining filter, this page has no reason to exist —
     hand them to the plain /journeys4 listing, carrying whatever else they'd
     already set (the sync effect above just wrote it to the URL, so we reuse
     that search string). Runs after the sync effect, so the URL is current.
  ...
  return (
    <div className="h26 jl jl2 jl4 jl4g">
      ...
      <section className="jl-hero jl2-hero">
        ...
        <div className="jl-hero-inner">
          ...
          <h1 className="jl-hero-title">
            <WordReveal text="Group Tours" accent={[1]} />
          </h1>
          <Reveal className="jl-hero-sub" as="p" delay={0.15}>
            Travel in a small group with everything handled — a tour manager throughout,
            fixed departures and like-minded company. Filter by where you're headed, what
            you'll spend and when you'll go; it's all on the left.
          </Reveal>
