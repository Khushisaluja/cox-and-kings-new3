/* ============================================================
   Journeys4Private — the PRIVATE / BESPOKE entry into the C&K listing.

   A sibling of /journeys4 and /journeys4-group. Below the hero it is the
   SAME page — the same full catalogue and the same open LEFT filter rail
   (Destination, Budget, Travellers, Dates, Pace, Trip style), the same
   filterable/sortable grid, badges and pill language, sharing Journeys4.css.
   Only two things differ: the hero speaks to private travellers ("Bespoke
   Tours"), and the page opens with the "Bespoke Private" trip-style filter
   already selected — so it lands on tailor-made journeys but a visitor can
   widen or change the filters freely.
...
export default function Journeys4Private() {
  ...
  /* Below the hero this page is identical to /journeys4 — same full catalogue,
     same sidebar. The one difference is that it opens with the "Bespoke Private"
     trip-style filter already selected. The URL still wins if it carries an
     explicit ?style=, so a shared/deep link stays authoritative. */
  const [styles, setStyles] = useState(() => {
    if (params.has('style')) return seedList('style', STYLES);
  ...
  /* "Bespoke Tours" is just /journeys4 opened on the Bespoke Private style. The
     moment a visitor removes that defining filter, this page has no reason to
     exist — hand them to the plain /journeys4 listing, carrying whatever else
     they'd already set (the sync effect above just wrote it to the URL, so we
     reuse that search string). Runs after the sync effect, so the URL is current.

     Reset scroll to the top BEFORE navigating: the visitor is almost always
     scrolled down into the results when they clear the filter, and /journeys4's
     reveal-on-scroll sections would otherwise mount already-scrolled and stay
     stuck at opacity 0 (the page looks half-rendered). Landing at y=0 lets every
     section animate in normally. */
  useEffect(() => {
    if (!styles.includes('Bespoke Private')) {
      window.scrollTo(0, 0);
      navigate(`/journeys4${window.location.search}`);
    }
  }, [styles, navigate]);
  ...
  return (
    <div className="h26 jl jl2 jl4 jl4p">
      ...
      <section className="jl-hero jl2-hero">
        ...
        <div className="jl-hero-inner">
          ...
          <h1 className="jl-hero-title">
            <WordReveal text="Bespoke Tours" accent={[1]} />
          </h1>
          <Reveal className="jl-hero-sub" as="p" delay={0.15}>
            Travel privately, on your own dates — every journey tailor-made around you,
            with a personal specialist, hand-picked stays and nothing shared. Filter by
            where you're headed, what you'll spend and when you'll go; it's all on the left.
          </Reveal>
